<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <Link class="btn-indigo" href="/admin/organizations/create">
        <span>Create</span>
      ...
      <table class="w-full whitespace-nowrap">
        ...
        <tbody>
          <tr v-for="organization in organizations.data" :key="organization.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
            <td class="border-t">
              <Link class="flex items-center px-6 py-4 focus:text-indigo-500" :href="`/admin/organizations/${organization.id}/edit`">
                {{ organization.name }}
                <icon v-if="organization.deleted_at" name="trash" class="shrink-0 ml-2 w-3 h-3 fill-gray-400" />
              </Link>
            </td>
            <td class="border-t">
              <Link class="flex items-center px-6 py-4" :href="`/admin/organizations/${organization.id}/edit`" tabindex="-1">
                {{ organization.city }}
              </Link>
            </td>
            <td class="border-t">
            ...
            <td class="w-px border-t">
              <Link class="flex items-center px-4" :href="`/admin/organizations/${organization.id}/edit`" tabindex="-1">
